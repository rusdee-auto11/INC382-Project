    ...
    public class FinancialReportController : ControllerBase
    {
        ...
        public IActionResult getIncomeStm()
        {
            try
            {
                ...
                double? SG = 0, SD = 0, COGSG = 0, COGSD = 0, SSOS = 0, SGC = 0, UE = 0, DP = 0;
                foreach(var item in values)
                {
                ...
                result.SaleGAS95 = SG;
                result.SaleDIESEL = SD;
                result.TotalSale = SG + SD;
                result.COGSGAS95 = COGSG;
                result.COGSDIESEL = COGSD;
                result.TotalCOGS = COGSG + COGSD;
                result.GrossProfit = result.TotalSale - result.TotalCOGS;
                result.SalarySOS = SSOS;
                result.SalaryGC = SGC;
                result.TotalSalary = SSOS + SGC;
                result.UtilityExp = UE;
                result.Depreciation = DP;
                result.NetIncome = result.GrossProfit - SSOS - SGC - UE - DP;
